-- Phase 1: Enhanced Profile Privacy - Create secure teammate profile access
...
-- Create a security function to check if user can view teammate profile data
CREATE OR REPLACE FUNCTION public.can_view_teammate_profile(profile_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE auth.uid() = ANY (t.members)
      AND profile_user_id = ANY (t.members)
      AND profile_user_id != auth.uid()  -- Can't view own profile through this (use direct policy)
  );
$$;

-- Create policy for teammates to view basic profile info only
CREATE POLICY "Team members can view teammate basic info"
ON public.profiles
FOR SELECT
TO authenticated
USING (
  -- User is viewing their own profile (full access)
  id = auth.uid() 
  OR 
  -- User is viewing teammate profile (restricted access via application logic)
  public.can_view_teammate_profile(id)
);
