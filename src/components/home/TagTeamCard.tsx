...
export const TagTeamCard: React.FC<TagTeamCardProps> = ({
  ...
  return <div className="border w-full mt-4 p-4 rounded-xl border-[rgba(130,122,255,0.41)] border-solid">
      ...
      <div className="flex items-center justify-between mt-2 my-[4px]">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className={`p-1 rounded-full ${isLogged ? 'bg-[#8CFF6E]/20' : 'bg-gray-100'}`}>
              {isLogged ? <Check className="w-4 h-4 text-[#8CFF6E]" /> : <X className="w-4 h-4 text-gray-400" />}
            </div>
            <div className={`p-1 rounded-full ${partnerLogged ? 'bg-[#8CFF6E]/20' : 'bg-gray-100'}`}>
              {partnerLogged ? <Check className="w-4 h-4 text-[#8CFF6E]" /> : <X className="w-4 h-4 text-gray-400" />}
            </div>
          </div>
