...
export const AppHeader = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('notification-changes').on('postgres_changes', {
      ...
      schema: 'public',
      table: 'notifications',
      filter: `user_id=eq.${supabase.auth.getUser().then(({
        data
      }) => data?.user?.id)}`
    }, () => {
      fetchNotifications();
      ...
      schema: 'public',
      table: 'team_requests',
      filter: `receiver_id=eq.${supabase.auth.getUser().then(({
        data
      }) => data?.user?.id)}`
    }, () => {
      fetchNotifications();
  ...
  return <div className="pt-[20px] pb-[11px] px-[15px] border-b py-[21px]">
      <div className="flex w-full items-center justify-between">
        <Link to="/">
          <img alt="TagTeam Logo" src="/lovable-uploads/4a1f663d-1562-4d37-81e5-ebab66ba6b0f.png" className="aspect-[3.27] w-[124px] object-scale-down" />
        </Link>
        <div className="relative">
          ...
          {unreadCount > 0 && <div className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
              {unreadCount}
            </div>}
        </div>
      </div>
