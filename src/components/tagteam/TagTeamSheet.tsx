          ...
          <div className="flex flex-col w-full h-full" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            ...
            <ScrollArea className="flex-1 px-4 pb-8">
              ...
              <div className="rounded-xl p-4 mb-4 mt-4 bg-[#f3f3f3]/[0.48]">
                <UserStatusSection firstUser={tagTeam.firstUser} secondUser={tagTeam.secondUser} timer={timer} timerColorClass={timerColorClass} />
                
                <TeamInfoSection interest={tagTeam.interest} frequency={tagTeam.frequency} />
                
                {loadingGoals ? <div className="min-h-[80px] p-4 rounded-md bg-white mb-4 flex items-center justify-center">
                    <p className="text-gray-400">Loading goals...</p>
                  </div> : <GoalSection activeGoal={activeGoal} setActiveGoal={setActiveGoal} currentUser={currentUser} partnerUser={partnerUser} onSetGoal={openGoalDialog} needsNewGoal={needsNewGoal} />}
              </div>
              ...
              <LeaveTagTeamButton tagTeamId={tagTeam.id} tagTeamName={tagTeam.name} partnerName={partnerUser.name} currentUserId={currentUserId} onLeaveComplete={onClose} />
            </ScrollArea>
            
            {/* Edit goal button positioned at bottom right */}
            {currentUserGoal && activeGoal === "your" && <div className="absolute bottom-10 right-5">
                <Button size="icon" className="h-10 w-10 rounded-full bg-gray-200 hover:bg-gray-300" onClick={openGoalDialog}>
