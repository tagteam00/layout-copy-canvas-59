...
export const PartnerStep: React.FC<PartnerStepProps> = ({
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      if (query.length >= 2) {
        filteredProfiles = profiles.filter(profile => (profile.full_name ?? "").toLowerCase().includes(query.toLowerCase()) || (profile.username ?? "").toLowerCase().includes(query.toLowerCase()));
      }
  ...
  return <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <Input type="text" value={searchQuery} onChange={e => handleSearch(e.target.value)} className="pl-10 w-full border border-[rgba(130,122,255,0.41)] rounded-xl" placeholder="Search by name or username" />
      </div>
      <div className="mt-4 space-y-2 max-h-[40vh] overflow-y-auto">
        {loading ? <div className="animate-pulse space-y-2">
            {[1, 2, 3].map(i => <div key={i} className="h-16 bg-gray-100 rounded-xl"></div>)}
          </div> : searchResults.length > 0 ? searchResults.map(user => <div key={user.id} className="p-4 border border-[rgba(130,122,255,0.41)] rounded-xl flex items-center justify-between hover:bg-[rgba(130,122,255,0.1)] transition-colors">
              ...
              <button onClick={() => handleSelectPartner(user.fullName, user.id)} className="flex items-center space-x-1 text-white px-3 py-1.5 rounded-lg transition-colors bg-gray-950 hover:bg-gray-800">
                
                <span>Select</span>
              </button>
            </div>) : <div className="text-center text-gray-500 py-4">
            {searchQuery.length > 0 ? "No users found matching your search" : `No users found with interest in ${selectedCategory}`}
          </div>}
      </div>
    </div>;
};